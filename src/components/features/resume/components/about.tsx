...
import { User, Mail, MapPin, Phone, Globe, Calendar, Linkedin } from 'lucide-react';
...
export function About({ aboutMe }: IAboutProps) {
    const getIconForField = (fieldName: string) => {
        ...
        if (field.includes('email') || field.includes('mail')) return <Mail className="w-4 h-4" />;
        if (field.includes('location') || field.includes('address')) return <MapPin className="w-4 h-4" />;
        if (field.includes('phone') || field.includes('mobile')) return <Phone className="w-4 h-4" />;
        if (field.includes('linkedin')) return <Linkedin className="w-4 h-4" />;
        if (field.includes('website') || field.includes('portfolio')) return <Globe className="w-4 h-4" />;
        if (field.includes('birth') || field.includes('age')) return <Calendar className="w-4 h-4" />;
        return <User className="w-4 h-4" />;
    };

    const formatFieldValue = (fieldName: string, fieldValue: string | undefined) => {
        ...
        if (field.includes('linkedin')) {
            ...
            return (
                ...
                >
                    <span className="hidden sm:inline">linkedin.com/in/</span>{username}
                </a>
            );
        }
        ...
        >
            ...
            <div className='text-center xl:text-left space-y-6'>
                <motion.h3
                    ...
                    className='text-4xl xl:text-5xl font-bold  bg-gradient-to-r from-foreground to-muted-foreground bg-clip-text text-transparent leading-tight'
                >
                    {aboutMe.title}
                </motion.h3>
                <motion.div
                            ...
                            <div className='flex items-center gap-6'>
                                ...
                                <div className='flex-1 space-y-2 min-w-0'>
                                    ...
                                    <div className='text-lg xl:text-xl font-bold text-foreground group-hover:text-accent transition-colors duration-300 leading-tight'>
                                        {item.fieldName && item.fieldValue && formatFieldValue(item.fieldName, item.fieldValue)}
                                    </div>
                                </div>
                            </div>
                        </div>
