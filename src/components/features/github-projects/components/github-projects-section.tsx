...
export function GitHubProjectsSection({ filter }: GitHubProjectsSectionProps) {
    ...
    const filterInfo = useMemo(() => {
        ...
        return services.find(service =>
            service.href.includes(`filter=${filter}`)
        );
    }, [filter]);
        ...
        return projects.filter(project => {
            ...
            if (matchesCurrentFilter) {
                ...
                const exclusionKeywords = {
                    mobile: ['backend', 'server', 'api', 'database', 'postgresql', 'mongodb', 'mysql'],
                    frontend: ['backend', 'server', 'api', 'database', 'postgresql', 'mongodb', 'mysql'],
                    backend: ['frontend', 'ui', 'react', 'angular', 'vue', 'svelte'],
                    fullstack: [] // Fullstack can include everything
                };

                const currentExclusions = exclusionKeywords[filter as keyof typeof exclusionKeywords] || [];
            ...
            <div className="container mx-auto">
                ...
                >
                    <h2 className="text-3xl xl:text-4xl font-bold text-foreground mb-4">
                        {filterInfo ? `${filterInfo.title} ` : ''}<span className="text-accent">Projects</span>
                    </h2>
